...
/***************version__tbl_param_user***************/
CREATE VIEW {schema}_v_param_cur AS 
 SELECT param__tbl.param_process AS param_cur_process,
   ...
   FROM {schema}_param__tbl param__tbl
     LEFT JOIN {schema}_param_sys param_sys ON param__tbl.param_process = param_sys.param_sys_process AND param__tbl.param_attrib = param_sys.param_sys_attrib
     LEFT JOIN {schema}_param_app param_app ON param__tbl.param_process = param_app.param_app_process AND param__tbl.param_attrib = param_app.param_app_attrib
     LEFT JOIN ( SELECT param_user_1.sys_user_id,
                   ...
                   FROM {schema}_param_user param_user_1
                  UNION
                 SELECT NULL AS sys_user_id,
                        param_user_null.param_user_process,
                        param_user_null.param_user_attrib,
                        NULL AS param_user_val
                   FROM {schema}_param_user param_user_null) param_user ON param__tbl.param_process = param_user.param_user_process AND param__tbl.param_attrib = param_user.param_user_attrib;

/***************v_app_info***************/
CREATE VIEW {schema}_v_app_info AS
 SELECT name.param_cur_val AS app_title,
   ...
   FROM ((((((((({schema}_single
     LEFT JOIN {schema}_v_param_cur name ON ((((name.param_cur_process) = 'HOUSE') AND ((name.param_cur_attrib) = 'NAME'))))
     LEFT JOIN {schema}_v_param_cur addr ON ((((addr.param_cur_process) = 'HOUSE') AND ((addr.param_cur_attrib) = 'ADDR'))))
     LEFT JOIN {schema}_v_param_cur city ON ((((city.param_cur_process) = 'HOUSE') AND ((city.param_cur_attrib) = 'CITY'))))
     LEFT JOIN {schema}_v_param_cur state ON ((((state.param_cur_process) = 'HOUSE') AND ((state.param_cur_attrib) = 'STATE'))))
     LEFT JOIN {schema}_v_param_cur zip ON ((((zip.param_cur_process) = 'HOUSE') AND ((zip.param_cur_attrib) = 'ZIP'))))
     LEFT JOIN {schema}_v_param_cur bphone ON ((((bphone.param_cur_process) = 'HOUSE') AND ((bphone.param_cur_attrib) = 'BPHONE'))))
     LEFT JOIN {schema}_v_param_cur fax ON ((((fax.param_cur_process) = 'HOUSE') AND ((fax.param_cur_attrib) = 'FAX'))))
     LEFT JOIN {schema}_v_param_cur email ON ((((email.param_cur_process) = 'HOUSE') AND ((email.param_cur_attrib) = 'EMAIL'))))
     LEFT JOIN {schema}_v_param_cur contact ON ((((contact.param_cur_process) = 'HOUSE') AND ((contact.param_cur_attrib) = 'CONTACT'))));
...
CREATE VIEW {schema}_code_param_app_process AS
...
CREATE VIEW {schema}_code_param_user_process AS
 SELECT DISTINCT null AS code_seq,
    param__tbl.param_process AS code_val,
    param__tbl.param_process AS code_txt,
    null AS code_code,
...
CREATE VIEW {schema}_code_param_sys_process AS
 SELECT DISTINCT null AS code_seq,
    param__tbl.param_process AS code_val,
    param__tbl.param_process AS code_txt,
    null AS code_code,
...
/***************v_audit_detail***************/
CREATE VIEW {schema}_v_audit_detail AS
 SELECT audit__tbl.audit_seq,
   ...
   FROM ({schema}_audit__tbl audit__tbl
     LEFT JOIN {schema}_audit_detail audit_detail ON ((audit__tbl.audit_seq = audit_detail.audit_seq)));
...
create trigger {schema}_v_doc_ext_update instead of update on {schema}_v_doc_ext
begin
  update {schema}_doc__tbl set doc_scope = new.doc_scope, doc_scope_id = new.doc_scope_id, doc_sts = new.doc_sts,
...
create trigger {schema}_v_doc_ext_delete instead of delete on {schema}_v_doc_ext
begin
  delete from {schema}_doc__tbl where doc_id = old.doc_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_doc_filename***************/
CREATE VIEW {schema}_v_doc_filename AS
 SELECT doc__tbl.doc_id,
    doc__tbl.doc_scope,
    doc__tbl.doc_scope_id,
    doc__tbl.cust_id,
...
/***************v_param_app***************/
CREATE VIEW {schema}_v_param_app AS
 SELECT param_app.param_app_id,
   ...
   FROM {schema}_param_app param_app;

create trigger {schema}_v_param_app_insert instead of insert on {schema}_v_param_app
begin
  insert into {schema}_param_app(param_app_process, param_app_attrib, param_app_val, param_app_etstmp, param_app_euser, param_app_mtstmp, param_app_muser)
...
create trigger {schema}_v_param_app_update instead of update on {schema}_v_param_app
begin
  update {schema}_param_app set param_app_process = new.param_app_process, param_app_attrib = new.param_app_attrib, param_app_val = new.param_app_val,
...
create trigger {schema}_v_param_app_delete instead of delete on {schema}_v_param_app
begin
  delete from {schema}_param_app where param_app_id = old.param_app_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_month***************/
CREATE VIEW {schema}_v_month AS
 SELECT number__tbl.number_val month_val,
    substr(('0' || number__tbl.number_val), -2, 2) AS month_txt
   FROM {schema}_number__tbl number__tbl
  WHERE (number__tbl.number_val <= 12);

/***************v_my_user***************/
CREATE VIEW {schema}_v_my_user AS
 SELECT {schema}.my_sys_user_id() AS my_sys_user_id;

/***************v_my_roles***************/
CREATE VIEW {schema}_v_my_roles AS
 SELECT sys_user_role.sys_role_name
   FROM {schema}_sys_user_role sys_user_role
  WHERE (sys_user_role.sys_user_id = {schema}.my_sys_user_id());

/***************v_note***************/
CREATE VIEW {schema}_v_note AS
...
/***************v_note_ext***************/
CREATE VIEW {schema}_v_note_ext AS
 SELECT note__tbl.note_id,
    note__tbl.note_scope,
    note__tbl.note_scope_id,
    note__tbl.note_sts,
...
create trigger {schema}_v_note_ext_insert instead of insert on {schema}_v_note_ext
begin
  insert into {schema}_note__tbl(note_scope, note_scope_id, note_sts, cust_id, item_id, note_type, note_body, note_etstmp, note_euser, note_mtstmp, note_muser, note_snotes)
...
create trigger {schema}_v_note_ext_update instead of update on {schema}_v_note_ext
begin
  update {schema}_note__tbl set note_scope = new.note_scope, note_scope_id = new.note_scope_id, note_sts = new.note_sts,
...
create trigger {schema}_v_note_ext_delete instead of delete on {schema}_v_note_ext
begin
  delete from {schema}_note__tbl where note_id = old.note_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_param***************/
CREATE VIEW {schema}_v_param AS
...
create trigger {schema}_v_param_insert instead of insert on {schema}_v_param
begin
  insert into {schema}_param__tbl(param_process, param_attrib, param_desc, param_type, code_name, is_param_app, is_param_user, is_param_sys, param_etstmp, param_euser, param_mtstmp, param_muser, param_snotes)
...
create trigger {schema}_v_param_update instead of update on {schema}_v_param
begin
  update {schema}_param__tbl set param_process = new.param_process, param_attrib = new.param_attrib, param_desc = new.param_desc,
...
create trigger {schema}_v_param_delete instead of delete on {schema}_v_param
begin
  delete from {schema}_param__tbl where param_id = old.param_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_param_user***************/
CREATE VIEW {schema}_v_param_user AS
 SELECT param_user.param_user_id,
   ...
   FROM {schema}_param_user param_user;

create trigger {schema}_v_param_user_insert instead of insert on {schema}_v_param_user
begin
  insert into {schema}_param_user(sys_user_id, param_user_process, param_user_attrib, param_user_val, param_user_etstmp, param_user_euser, param_user_mtstmp, param_user_muser)
...
create trigger {schema}_v_param_user_update instead of update on {schema}_v_param_user
begin
  update {schema}_param_user set sys_user_id = new.sys_user_id, param_user_process = new.param_user_process, param_user_attrib = new.param_user_attrib, param_user_val = new.param_user_val,
...
create trigger {schema}_v_param_user_delete instead of delete on {schema}_v_param_user
begin
  delete from {schema}_param_user where param_user_id = old.param_user_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_sys_menu_role_selection***************/
CREATE VIEW {schema}_v_sys_menu_role_selection AS
 SELECT sys_menu_role.sys_menu_role_id,
   ...
   FROM ((( SELECT sys_role.sys_role_name,
            ...
            menu__tbl.menu_snotes,
            menu__tbl.menu_subcmd
           FROM ({schema}_sys_role sys_role
             LEFT JOIN {schema}_menu__tbl menu__tbl ON (menu__tbl.menu_group = 'S'))) m
     JOIN {schema}_single single ON (1 = 1))
     LEFT JOIN {schema}_sys_menu_role sys_menu_role ON (((sys_menu_role.sys_role_name = m.sys_role_name) AND (sys_menu_role.menu_id = m.menu_id))));

create trigger {schema}_v_sys_menu_role_selection_update instead of update on {schema}_v_sys_menu_role_selection
begin
  delete from {schema}_sys_menu_role where sys_menu_role_id=new.sys_menu_role_id and (%%%nequal("NEW.sys_menu_role_selection","OLD.sys_menu_role_selection")%%%) and coalesce(new.sys_menu_role_selection,0)=0\;
  insert into {schema}_sys_menu_role (sys_role_name, menu_id)
...
/***************v_param_sys***************/
CREATE VIEW {schema}_v_param_sys AS
 SELECT param_sys.param_sys_id,
   ...
   FROM {schema}_param_sys param_sys;

create trigger {schema}_v_param_sys_insert instead of insert on {schema}_v_param_sys
begin
  insert into {schema}_param_sys(param_sys_process, param_sys_attrib, param_sys_val, param_sys_etstmp, param_sys_euser, param_sys_mtstmp, param_sys_muser)
...
create trigger {schema}_v_param_sys_update instead of update on {schema}_v_param_sys
begin
  update {schema}_param_sys set param_sys_process = new.param_sys_process, param_sys_attrib = new.param_sys_attrib, param_sys_val = new.param_sys_val,
...
create trigger {schema}_v_param_sys_delete instead of delete on {schema}_v_param_sys
begin
  delete from {schema}_param_sys where param_sys_id = old.param_sys_id\;
  update jsharmony_meta set extra_changes=extra_changes+1\;
end;

/***************v_year***************/
CREATE VIEW {schema}_v_year AS
