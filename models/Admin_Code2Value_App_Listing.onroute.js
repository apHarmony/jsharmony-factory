...
sql_code_type = " and code_type='app' ";
//:endif:
jsh.AppSrv.ExecRow(req._DBContext, "select code_desc,code_code_desc,code_attrib_desc from {schema}.code2_app where code_name=@code_name "+sql_code_type+" and coalesce(code_schema,'')=coalesce(@code_schema,'')", [dbtypes.VarChar(128),dbtypes.VarChar(128)], { 'code_name': code_name, 'code_schema': code_schema }, function (err, rslt) {
  if (err) { jsh.Log.error(err); Helper.GenError(req, res, -99999, "An unexpected error has occurred"); return; }
  if (rslt && rslt.length && rslt[0]) {
    //Set title
    model.title = 'TABLE - '+rslt[0]['code_desc'];
    //Set table
    model.table = 'code2_app_'+code_name;
    if(code_schema){ 
